...
读数据流: 
  ...
  $  000.ECM(发动机控制模块)节点的通讯配置            $    $        $    $  ANS000.BYTE004  $  if((x&0x80)==0x80)y=@00dc;else y=@00db;
  $  001.TCU(变速箱控制单元)节点的通讯配置            $    $        $    $  ANS000.BYTE004  $  if((x&0x20)==0x20)y=@00dc;else y=@00db;
  $  002.LCA(变道辅助系统)节点的通讯配置              $    $        $    $  ANS000.BYTE005  $  if((x&0x80)==0x80)y=@00dc;else y=@00db;
  $  003.ABS/ESP节点的通讯配置                        $    $        $    $  ANS000.BYTE005  $  if((x&0x20)==0x20)y=@00dc;else y=@00db;
  $  004.SAS(方向盘转向角传感器)节点的通讯配置        $    $        $    $  ANS000.BYTE006  $  if((x&0x10)==0x10)y=@00dc;else y=@00db;
  $  005.TPMS(胎压管理系统)节点的通讯配置             $    $        $    $  ANS000.BYTE007  $  if((x&0x40)==0x40)y=@00dc;else y=@00db;
  $  006.BCM(车身控制管理系统)节点的通讯配置          $    $        $    $  ANS000.BYTE007  $  if((x&0x20)==0x20)y=@00dc;else y=@00db;
  $  007.FICM(娱乐系统大屏含导航)节点的通讯配置       $    $        $    $  ANS000.BYTE008  $  if((x&0x80)==0x80)y=@00dc;else y=@00db;
  $  008.IPK(仪表)节点的通讯配置                      $    $        $    $  ANS000.BYTE008  $  if((x&0x40)==0x40)y=@00dc;else y=@00db;
  $  009.TBOX(远程监控模块)节点的通讯配置             $    $        $    $  ANS000.BYTE009  $  if((x&0x80)==0x80)y=@00dc;else y=@00db;
  $  010.U1562-系统蓄电池电压过高                     $    $        $    $  ANS001.BYTE004  $  if((x&0x80)==0x80)y=@055b;else y=@055c;
  $  011.U1563-系统蓄电池电压过低                     $    $        $    $  ANS001.BYTE004  $  if((x&0x40)==0x40)y=@055b;else y=@055c;
  $  012.U0073-PT CAN上的控制器通讯总线关闭           $    $        $    $  ANS001.BYTE004  $  if((x&0x20)==0x20)y=@055b;else y=@055c;
  $  013.U0074-CH CAN上的控制器通讯总线关闭           $    $        $    $  ANS001.BYTE004  $  if((x&0x10)==0x10)y=@055b;else y=@055c;
  $  014.U0075-BD CAN上的控制器通讯总线关闭           $    $        $    $  ANS001.BYTE004  $  if((x&0x08)==0x08)y=@055b;else y=@055c;
  $  015.U0076-Info CAN上的控制器通讯总线关闭         $    $        $    $  ANS001.BYTE004  $  if((x&0x04)==0x04)y=@055b;else y=@055c;
  $  016.U0077-Diag CAN上的控制器通讯总线关闭         $    $        $    $  ANS001.BYTE004  $  if((x&0x02)==0x02)y=@055b;else y=@055c;
  $  017.U2002-ECU 存储器校验总和错误                 $    $        $    $  ANS001.BYTE009  $  if((x&0x80)==0x80)y=@055b;else y=@055c;
  $  018.U2002-ECU 内存错误                           $    $        $    $  ANS001.BYTE009  $  if((x&0x40)==0x40)y=@055b;else y=@055c;
  $  019.B162F-存储器错误                             $    $        $    $  ANS001.BYTE009  $  if((x&0x20)==0x20)y=@055b;else y=@055c;
  $  020.硬线输入信号HDI1(KL.15 RUN)状态              $    $        $    $  ANS002.BYTE004  $  if((x&0x80)==0x80)y=@0028;else y=@000f;
  $  021.硬线输入信号HDI2(KL.R ACC)状态               $    $        $    $  ANS002.BYTE004  $  if((x&0x40)==0x40)y=@0028;else y=@000f;
  $  022.硬线输入信号HDI3(预留)状态                   $    $        $    $  ANS002.BYTE004  $  if((x&0x20)==0x20)y=@0028;else y=@000f;
  $  023.硬线输入信号HDI4(预留)状态                   $    $        $    $  ANS002.BYTE004  $  if((x&0x10)==0x10)y=@0028;else y=@000f;
  $  024.EEPROM校验检查状态                           $    $        $    $  ANS003.BYTE004  $  if(x==0x00)y=@00c9;else if(x==0x01)y=@00ca;else y=@0004;
  $  025.电源KL.30模拟采集值                          $    $  V     $    $  ANS004.BYTE004  $  y=(x1*256+x2)*0.0276+0.42;

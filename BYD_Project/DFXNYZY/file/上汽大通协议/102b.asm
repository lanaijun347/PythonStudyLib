...
读数据流: 
  ...
  $  004.PTG节点的通讯配置                      $    $        $    $  ANS000.BYTE004  $  if((x&0x04)==0x04)y=@00dc;else y=@00db;
  $  005.LPSD左侧滑移门节点的通讯配置           $    $        $    $  ANS000.BYTE004  $  if((x&0x02)==0x02)y=@00dc;else y=@00db;
  $  006.RPSD右侧滑移门节点的通讯配置           $    $        $    $  ANS000.BYTE004  $  if((x&0x01)==0x01)y=@00dc;else y=@00db;
  $  007.PD CAN通道通讯                         $    $        $    $  ANS001.BYTE004  $  if((x&0x80)==0x80)y=@00dc;else y=@00db;
  $  008.FICM节点的通讯配置                     $    $        $    $  ANS001.BYTE005  $  if((x&0x20)==0x20)y=@00dc;else y=@00db;
  $  009.AD CAN通道通讯                         $    $        $    $  ANS001.BYTE004  $  if((x&0x20)==0x20)y=@00dc;else y=@00db;
  $  010.VD CAN通道通讯                         $    $        $    $  ANS001.BYTE004  $  if((x&0x40)==0x40)y=@00dc;else y=@00db;
  $  011.硬线输入信号HDI1(KL.15 RUN)状态        $    $        $    $  ANS002.BYTE004  $  if((x&0x80)==0x80)y=@00b2;else y=@00b1;
  $  012.硬线输入信号HDI2(KL.R ACC)状态         $    $        $    $  ANS002.BYTE004  $  if((x&0x40)==0x40)y=@00b2;else y=@00b1;
  $  013.硬线输入信号HDI3(KL.15 预留)状态       $    $        $    $  ANS002.BYTE004  $  if((x&0x20)==0x20)y=@00b2;else y=@00b1;
  $  014.EEPROM校验检查状态                     $    $        $    $  ANS003.BYTE004  $  if((x&0x01)==0x01)y=@00ca;else y=@00c9;
  $  015.内部VCC(电源电压)上拉信号状态          $    $        $    $  ANS004.BYTE004  $  if((x&0x80)==0x80)y=@0028;else y=@000f;
  $  016.电源KL.30模拟采集值                    $    $  V     $    $  ANS005.BYTE004  $  y=(x1*256+x2)*0.0276+0.42;

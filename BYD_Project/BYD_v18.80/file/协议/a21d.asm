...
版本信息:
  ...
  $%  000:硬件号:              $%    $%  ANS000.BYTE004  $%  y=SPRINTF([%02X.%02X.%02X %d%s%d%s%d%s],x1,x2,x3,x4,@0038,x5,@0039,x6,@003a);
  $%  001:软件号:              $%    $%  ANS001.BYTE004  $%  y=SPRINTF([%02X.%02X.%02X %d%s%d%s%d%s],x1,x2,x3,x4,@0038,x5,@0039,x6,@003a);
  $%  002:序列号:              $%    $%  ANS002.BYTE004  $%  HEX(x1,x2,x3,x4,x5,x6);
  $%  003:当前复位状态:        $%    $%  ANS003.BYTE004  $%  if(x1&0x80)y=@0864;else if(x1&0x40)y=@0865;else if(x1&0x20) y=@0866;else if(x1&0x10)y=@0867;else if(x1&0x08)y=@0868;else if(x1&0x04)y=@0869;else if(x1&0x02)y=@086a;else y=@007f;

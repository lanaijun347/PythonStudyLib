
    车型ID：0a03

    模拟：协议模拟-->0a03

;******************************************************************************************************************************************************

    通讯线: $~06$~14$~500k$~78F

进入命令:

  $~  REQ000:787 02 10 01 00 00 00 00 00       $~  ANS000:78F 02 10 01 00 00 00 00 00

空闲命令:

  $!  REQ000:787 02 3E 80 00 00 00 00 00       $!  ANS000:78F 02 3E 80 00 00 00 00 00

退出命令:



;******************************************************************************************************************************************************

读取故障码:

  $#  REQ000:787 05 18 00 FF FF FF 00 00       $#  ANS000:78F 05 18 00 FF FF FF 00 00

  ANS000.BYTE02 控制故障码个数，从$#ANS000.BYTE3$#开始每$#4$#个字节表示一个故障码，前$#3$#表示码号。

  码库:

  $*$*DTC/83000000


<RDTC>
  <FUNCTION type="0" num="0">
     <param type="string" value="1"/>
   </FUNCTION>
</RDTC>

;******************************************************************************************************************************************************

清除故障码:

  $$  REQ000:787 04 14 FF FF FF 00 00 00       $$  ANS000:78F 04 14 FF FF FF 00 00 00

;******************************************************************************************************************************************************

版本信息:

  $%  REQ000:787 03 22 00 01 00 00 00 00       $%  ANS000:78F 03 22 00 01 00 00 00 00
  $%  REQ001:787 03 22 00 03 00 00 00 00       $%  ANS001:78F 03 22 00 03 00 00 00 00

  $%  000:硬件日期:        $%    $%  ANS000.BYTE004  $%  y=SPRINTF([20%02d%s%02d%s%02d%s],x1,@0038,x2,@0039,x3,@003a);
  $%  001:硬件版本:        $%    $%  ANS000.BYTE007  $%  y=SPRINTF([%02d.%02d.%02d],x1,x2,x3);
  $%  002:软件日期:        $%    $%  ANS001.BYTE004  $%  y=SPRINTF([20%02d%s%02d%s%02d%s],x1,@0038,x2,@0039,x3,@003a);
  $%  003:软件版本:        $%    $%  ANS001.BYTE007  $%  y=SPRINTF([%02d.%02d.%02d],x1,x2,x3);

;******************************************************************************************************************************************************

读数据流: 

  $  REQ000:787 03 22 00 04 00 00 00 00       $  ANS000:78F 03 22 00 04 00 00 00 00
  $  REQ001:787 03 22 00 05 00 00 00 00       $  ANS001:78F 03 22 00 05 00 00 00 00
  $  REQ002:787 03 22 00 06 00 00 00 00       $  ANS002:78F 03 22 00 06 00 00 00 00

  $  000.故障检测标志位         $    $       $    $  ANS000.BYTE004  $  if(x==0x01)y=@03c3;else if(x==0x02)y=@03c4;else if(x==0x03)y=@03c5;else if(x==0x04)y=@03c6;else if(x==0x05)y=@0122;else if(x==0x06)y=@0123;else y=@0108;
  $  001.故障检测持续判断       $    $       $    $  ANS001.BYTE004  $  if(x==0x01)y=@0125;else if(x==0x02)y=@0126;else if(x==0x03)y=@0127;else if(x==0x04)y=@0128;else y=@0108;
  $  002.自身识别档位消息       $    $       $    $  ANS002.BYTE004  $  if(x==0x00)y=@0124;else if(x==0x01)y=@0073;else if(x==0x02)y=@0074;else if(x==0x03)y=@0075;else if(x==0x04)y=@0076;else y=@0108;

;******************************************************************************************************************************************************

30帧:

  $FC REQ000:787 30 01 04 00 00 00 00 00  $FC 

;******************************************************************************************************************************************************

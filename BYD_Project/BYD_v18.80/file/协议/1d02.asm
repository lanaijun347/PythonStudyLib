...
版本信息:
  ...
  $%  002:生产日期(0):               $%    $%  ANS000.BYTE007  $%  y=SPRINTF([20%02d%s%02d%s%02d%s],x1,@0038,x2,@0039,x3,@003a);
  $%  003:生产日期(1):               $%    $%  ANS000.BYTE010  $%  y=SPRINTF([20%02d%s%02d%s%02d%s],x1,@0038,x2,@0039,x3,@003a);
  $%  004:生产日期(2):               $%    $%  ANS000.BYTE013  $%  y=SPRINTF([20%02d%s%02d%s%02d%s],x1,@0038,x2,@0039,x3,@003a);
  $%  005:生产日期(3):               $%    $%  ANS000.BYTE016  $%  y=SPRINTF([20%02d%s%02d%s%02d%s],x1,@0038,x2,@0039,x3,@003a);
  $%  006:生产日期(4):               $%    $%  ANS000.BYTE019  $%  y=SPRINTF([20%02d%s%02d%s%02d%s],x1,@0038,x2,@0039,x3,@003a);
  $%  007:生产序列号:                $%    $%  ANS001.BYTE004  $%  y=SPRINTF([%02X %02X %02X %02X %02X %02X],x1,x2,x3,x4,x5,x6);
  $%  008:车辆规格:                  $%    $%  ANS002.BYTE004  $%  if(x1==0x01)y=@0337;else if(x1==0x02)y=@0338;else if(x1==0x03)y=@0339;else if(x1==0x04)y=@033a;else if(x1==0x05)y=@033b;else if(x1==0x06)y=@033c;else y=@0002;
  $%  009:高度传感器标定结果:        $%    $%  ANS003.BYTE004  $%  if(x1==0x00)y=@0093;else if(x1==0x01)y=@003f;else y=@0002;
...
读数据流: 
  ...
  $  000.近光灯状态           $    $        $    $  ANS000.BYTE004  $  if(x1==0x00)y=@002d;else if(x1==0x01)y=@0000;else y=@0002;
  ...
  $  006.前灯调节器状态       $    $        $    $  ANS006.BYTE004  $  if((x1==0x00)&&(x2==0x00))y=@0341;else if((x1==0x00)&&(x2==0x01))y=@0342;else if((x1==0x00)&&(x2==0x02))y=@0343;else if((x1==0x00)&&(x2==0x03))y=@0035;else y=@0002;

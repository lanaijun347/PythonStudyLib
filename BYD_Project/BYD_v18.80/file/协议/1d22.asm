...
版本信息:
  ...
  $%  002:软件最后更新日期:        $%    $%  ANS000.BYTE007  $%  y=SPRINTF([%dY%dM%dD],x1,x2,x3);
  $%  003:生产日期:                $%    $%  ANS001.BYTE004  $%  y=SPRINTF([%dY%dM%dD],x1,x2,x3);
  $%  004:PTC产品序列号:           $%    $%  ANS002.BYTE004  $%  y=SPRINTF([%02X%02X%02X],x1,x2,x3);
  $%  005:生产日期:                $%    $%  ANS002.BYTE007  $%  y=SPRINTF([%dY%dM%dD],x1,x2,x3);
...
读数据流: 
  ...
  $  003.右侧散热片温度        $    $  degree C     $    $  ANS003.BYTE004  $  if(x1<171) y=x1;else y=@0002;
  $  004.冷却液温度            $    $  degree C     $    $  ANS004.BYTE004  $  if(x1<171) y=x1;else y=@0002;

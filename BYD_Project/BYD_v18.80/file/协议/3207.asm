...
读数据流: 
  ...
  $  013.OK灯状态                        $    $           $    $  ANS013.BYTE004  $  if(x1==0) y=@02bf;else if(x1==1) y=@02c0;else y=@0002;
  $  014.真空泵状态                      $    $           $    $  ANS014.BYTE004  $  if(x1==0) y=@04c0;else if(x1==1) y=@04c1;else y=@0002;
  $  015.风扇高速继电器状态              $    $           $    $  ANS015.BYTE004  $  if(x1==0) y=@05b0;else if(x1==1) y=@05b1;else if(x1==255) y=@04c2;else y=@0002;
  $  016.风扇低速继电器状态              $    $           $    $  ANS016.BYTE004  $  if(x1==0) y=@05ae;else if(x1==1) y=@05af;else if(x1==255) y=@04c2;else y=@0002;
  $  017.坡道坡度                        $    $           $    $  ANS017.BYTE004  $  if(((x2*256+x1)>=0)&&((x2*256+x1)<=200)) y=(x2*256+x1)-100;else if((x2*256+x1)==32767) y=@04c3;else y=@02ae;
  $  018.真空压力值                      $    $  kpa      $    $  ANS018.BYTE004  $  y=x1;
  $  019.水温报警                        $    $           $    $  ANS019.BYTE004  $  if(x1==0) y=@022f;else if(x1==1) y=@0230;else if(x1==255) y=@04c4;else y=@0002;
  $  020.真空压力报警                    $    $           $    $  ANS020.BYTE004  $  if(x1==0) y=@0045;else if(x1==1) y=@0231;else if(x1==2) y=@0232;else y=@0002;
  $  021.真空泵工作时间                  $    $  Min      $    $  ANS021.BYTE004  $  y=(x4*256*256*256+x3*256*256+x2*256+x1);
  $  022.真空泵继电器状态                $    $           $    $  ANS022.BYTE004  $  if(x1==0) y=@0045;else if(x1==1) y=@04c5;else if(x1==2) y=@04c6;else if(x1==3) y=@04c7;else y=@0002;
  $  023.整车车速                        $    $  km/h     $    $  ANS023.BYTE004  $  if(((x2*256+x1)>=0)&&((x2*256+x1)<=3000)) y=SPRINTF([%.1f],(x2*256+x1)*0.1);else if((x2*256+x1)==65535) y=@00c7;else y=@0002;
  $  024.总里程                          $    $  km       $    $  ANS024.BYTE004  $  y=SPRINTF([%.1f],(x3*256*256+x2*256+x1)/10);
  $  025.倾角标定工作信息/是否校准       $    $           $    $  ANS025.BYTE004  $  if((x2*256+x1)==0) y=@00ec;else if((x2*256+x1)==1) y=@00ed;else y=@02ae;
  $  026.油门深度电压1                   $    $  V        $    $  ANS026.BYTE004  $  y=SPRINTF([%.3f],(x2*256+x1)/1000);
  $  027.油门深度电压2                   $    $  V        $    $  ANS027.BYTE004  $  y=SPRINTF([%.3f],(x2*256+x1)/1000);
  $  028.制动开关状态                    $    $           $    $  ANS028.BYTE004  $  if((x2*256+x1)==0) y=@00c0;else if((x2*256+x1)==1) y=@0003;else y=@0002;
  $  029.VIN是否写入                     $    $           $    $  ANS029.BYTE004  $  if((x2*256+x1)==0) y=@0265;else if((x2*256+x1)==1) y=@04bf;else y=@0002;

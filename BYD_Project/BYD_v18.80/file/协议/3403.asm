...
读数据流: 
  ...
  $  008.EVAP燃油蒸发系统监测完成次数            $    $  次     $    $  ANS000.BYTE020  $  y=(x1*256+x2);
  $  009.EVAP燃油蒸发系统监测条件满足次数        $    $  次     $    $  ANS000.BYTE022  $  y=(x1*256+x2);
...
$^TIP:操作说明： 1、 发动机冷却剂温度低于特定阀值（≤100℃），否则ECM 将启动冷却风扇防止发动机过热 2、测试时不要求空调请求 冷却风扇开启时的无条件限制 
;----------------------------------------------------------------------------------------------------

...
版本信息:
  ...
  $%  000:硬件号:              $%    $%  ANS000.BYTE004  $%  y=SPRINTF([%02X.%02X.%02X %d%s%d%s%d%s],x1,x2,x3,x4,@001d,x5,@001e,x6,@001f);
  $%  001:软件号:              $%    $%  ANS001.BYTE004  $%  y=SPRINTF([%02X.%02X.%02X %d%s%d%s%d%s],x1,x2,x3,x4,@001d,x5,@001e,x6,@001f);
  $%  002:序列号:              $%    $%  ANS002.BYTE004  $%  HEX(x1,x2,x3,x4,x5,x6);
  $%  003:当前复位状态:        $%    $%  ANS003.BYTE004  $%  if(x1&0x80)y=@02eb;else if(x1&0x40)y=@02ec;else if(x1&0x20) y=@02ed;else if(x1&0x10)y=@02ee;else if(x1&0x08)y=@02ef;else if(x1&0x04)y=@02f0;else if(x1&0x02)y=@02f1;else y=@0076;

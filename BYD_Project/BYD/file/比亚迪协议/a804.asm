
    车型ID：a804

    模拟：协议模拟-->a804

;******************************************************************************************************************************************************

    通讯线: $~06$~14$~500k

进入命令:

  $~  REQ000:730 02 10 01 00 00 00 00 00       $~  ANS000:738 02 10 01 00 00 00 00 00

空闲命令:

  $!  REQ000:730 02 3E 01 00 00 00 00 00       $!  ANS000:738 02 3E 01 00 00 00 00 00

退出命令:



;******************************************************************************************************************************************************

读取故障码:

  $#  REQ000:730 04 18 00 00 FF 00 00 00       $#  ANS000:738 04 18 00 00 FF 00 00 00

  ANS000.BYTE02 控制故障码个数，从$#ANS000.BYTE3$#开始每$#3$#个字节表示一个故障码，前$#2$#表示码号。

  码库:

  $*$*DTC/a8020000_2BYTE


<RDTC>
  <FUNCTION type="0" num="0">
     <param type="string" value="1"/>
   </FUNCTION>
</RDTC>

;******************************************************************************************************************************************************

清除故障码:

  $$  REQ000:730 03 14 00 FF 00 00 00 00       $$  ANS000:738 03 14 00 FF 00 00 00 00

;******************************************************************************************************************************************************

版本信息:

  $%  REQ000:730 03 22 00 01 00 00 00 00       $%  ANS000:738 03 22 00 01 00 00 00 00
  $%  REQ001:730 03 22 00 05 00 00 00 00       $%  ANS001:738 03 22 00 05 00 00 00 00

  $%  000:技术方案与生产阶段:        $%    $%  ANS000.BYTE006  $%  HEX(x1);
  $%  001:软件号:                    $%    $%  ANS000.BYTE007  $%  if(x1%16<10)y=x1/16+x1%16*0.1;else y=x1/16+x1%16*0.01;
  $%  002:硬件号:                    $%    $%  ANS000.BYTE008  $%  if(x1%16<10)y=x1/16+x1%16*0.1;else y=x1/16+x1%16*0.01;
  $%  003:驱动数:                    $%    $%  ANS000.BYTE005  $%  HEX(x1);
  $%  004:软件日期:                  $%    $%  ANS000.BYTE009  $%  y=SPRINTF([%02X-%02X-%02X],x1,x2,x3);
  $%  005:生产日期:                  $%    $%  ANS001.BYTE004  $%  y=SPRINTF([%d-%d-%d],x1,x2,x3);
  $%  006:维修日期:                  $%    $%  ANS001.BYTE007  $%  y=SPRINTF([%d-%d-%d],x1,x2,x3);
  $%  007:维修次数:                  $%    $%  ANS001.BYTE010  $%  y=x1;

;******************************************************************************************************************************************************

30帧:

  $FC REQ000:730 30 00 00 00 00 00 00 00  $FC 

;******************************************************************************************************************************************************

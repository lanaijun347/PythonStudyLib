...
读数据流: 
  ...
  $  027.主缸压力值                       $    $  bar       $    $  ANS008.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],(x1*256+x2)*0.0153);else y=SPRINTF([%.1f],((x1-256)*256+x2)*0.0153);
  $  028.转角传感器                       $    $  °        $    $  ANS008.BYTE006  $  if(x1<0x80) y=SPRINTF([%.2f],(x1*256+x2)*0.1);else y=SPRINTF([%.2f],((x1-256)*256+x2)*0.1);
  $  029.偏航率传感器                     $    $  rad/s     $    $  ANS008.BYTE008  $  if(x1<0x80) y=SPRINTF([%.3f],(x1*256+x2)*0.00213);else y=SPRINTF([%.3f],((x1-256)*256+x2)*0.00213);
  $  030.横向加速度传感器                 $    $  m/s^2     $    $  ANS008.BYTE010  $  if(x1<0x80) y=SPRINTF([%.3f],(x1*256+x2)*0.02712);else y=SPRINTF([%.3f],((x1-256)*256+x2)*0.02712);
  $  031.纵向加速度传感器                 $    $  m/s^2     $    $  ANS008.BYTE012  $  if(x1<0x80) y=SPRINTF([%.3f],(x1*256+x2)*0.02712);else y=SPRINTF([%.3f],((x1-256)*256+x2)*0.02712);

...
读数据流: 
  ...
  $  000.系统工作模式                        $    $            $    $  ANS000.BYTE004  $  if(x1==1) y=@0278;else if(x1==2) y=@0279;else if(x1==3) y=@027a;else if(x1==4) y=@027b;else y=@0001;
  $  001.蓄电池电压                          $    $  deg       $    $  ANS001.BYTE004  $  y=SPRINTF([%.1f],x1/10);
  $  002.方向盘力矩                          $    $  NM        $    $  ANS002.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],x1/10);else y=SPRINTF([%.1f],(x1-256)/10);
  $  003.方向盘转角                          $    $  deg       $    $  ANS003.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],(x1*256+x2)/10);else y=SPRINTF([%.1f],((x1-256)*256+x2)/10);
  $  004.电机温度                            $    $  ℃        $    $  ANS004.BYTE004  $  y=SPRINTF([%d],(x1*256+x2)*2);
  $  005.ECU温度                             $    $  ℃        $    $  ANS005.BYTE004  $  y=x1;
  $  006.车速                                $    $  Km/h      $    $  ANS006.BYTE004  $  y=x1;
  $  007.扭矩传感器电压                      $    $  V         $    $  ANS007.BYTE004  $  y=SPRINTF([%d],x1*0.1);
  $  008.电机电流                            $    $  A         $    $  ANS008.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],x1);else y=SPRINTF([%d],(x1-256));
  $  009.方向盘转速                          $    $  deg/s     $    $  ANS009.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],(x1*256+x2)*0.1);else y=SPRINTF([%d],((x1-256)*256+x2)*0.1);

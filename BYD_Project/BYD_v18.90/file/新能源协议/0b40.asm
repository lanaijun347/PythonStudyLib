
    车型ID：0b40

    模拟：协议模拟-->0b40

;******************************************************************************************************************************************************

    通讯线: $~06$~14$~500k$~7CF

进入命令:

  $~  REQ000:7C7 02 10 01 00 00 00 00 00       $~  ANS000:7CF 02 10 01 00 00 00 00 00
  $~  REQ001:7C7 02 10 01 00 00 00 00 00       $~  ANS001:7CF 02 10 01 00 00 00 00 00

空闲命令:

  $!  REQ000:7C7 02 3E 80 00 00 00 00 00       $!  ANS000:7CF 02 3E 80 00 00 00 00 00

退出命令:



;******************************************************************************************************************************************************

读取故障码:

  $#  REQ000:7C7 04 18 00 00 FF 00 00 00       $#  ANS000:7CF 04 18 00 00 FF 00 00 00

		控制公式：
			y=(x1<<8)+x2;

  ANS000.BYTE02 控制故障码个数，从$#ANS000.BYTE3$#开始每$#3$#个字节表示一个故障码，前$#2$#表示码号。

  码库:

  $*$*DTC/feff0000_2BYTE


<RDTC>
  <FUNCTION type="0" num="0">
     <param type="string" value="1"/>
   </FUNCTION>
</RDTC>

;******************************************************************************************************************************************************

清除故障码:

  $$  REQ000:7C7 03 14 00 FF 00 00 00 00       $$  ANS000:7CF 03 14 00 FF 00 00 00 00

;******************************************************************************************************************************************************

版本信息:

  $%  REQ000:7C7 03 22 00 01 00 00 00 00       $%  ANS000:7CF 03 22 00 01 00 00 00 00

  $%  000:软件版本:        $%    $%  ANS000.BYTE004  $%  y=SPRINTF([%d.%d],x1/10,x1%10);
  $%  001:硬件版本:        $%    $%  ANS000.BYTE005  $%  y=SPRINTF([%d.%d],x1/10,x1%10);
  $%  002:硬件日期:        $%    $%  ANS000.BYTE007  $%  y=SPRINTF([%d%s%d%s%d%s],x1,@0197,x2,@0198,x3,@0199);

;******************************************************************************************************************************************************

读数据流: 

  $  REQ000:7C7 03 22 00 04 00 00 00 00       $  ANS000:7CF 03 22 00 04 00 00 00 00
  $  REQ001:7C7 03 22 00 05 00 00 00 00       $  ANS001:7CF 03 22 00 05 00 00 00 00

  $  000.管柱调节开关上倾信号               $    $        $    $  ANS000.BYTE004  $  if(x1&0x01)y=@001d;else y=@001e;
  $  001.管柱调节开关下倾信号               $    $        $    $  ANS000.BYTE004  $  if(x1&0x02)y=@001d;else y=@001e;
  $  002.管柱调节开关伸出信号               $    $        $    $  ANS000.BYTE004  $  if(x1&0x04)y=@001d;else y=@001e;
  $  003.管柱调节开关缩回信号               $    $        $    $  ANS000.BYTE004  $  if(x1&0x08)y=@001d;else y=@001e;
  $  004.电动管柱系统状态                   $    $        $    $  ANS000.BYTE004  $  if(x1&0x10)y=@0087;else y=@0078;
  $  005.电动管柱工作状态                   $    $        $    $  ANS000.BYTE004  $  if(x1&0x20)y=@0006;else if(x1&0x40) y=@0ab4;else if(x1&0x80) y=@0ab5;else y=@0525;
  $  006.电源档位状态                       $    $        $    $  ANS000.BYTE005  $  if(x1==0x00)y=@001d;else if(x1==0x01) y=@0011;else if(x1==0x02) y=@001e;else y=@0525;
  $  007.电源电压                           $    $  V     $    $  ANS001.BYTE004  $  y=x1*19.89/255;
  $  008.电动管柱轴向调节电机当前位置       $    $        $    $  ANS001.BYTE005  $  y=x1*256+x2;
  $  009.电动管柱角度调节电机当前位置       $    $        $    $  ANS001.BYTE007  $  y=x1*256+x2;
  $  010.电动管柱轴向调节电机行程           $    $        $    $  ANS001.BYTE009  $  y=x1*256+x2;
  $  011.电动管柱角度调节电机行程           $    $        $    $  ANS001.BYTE011  $  y=x1*256+x2;

;******************************************************************************************************************************************************

30帧:

  $FC REQ000:7C7 30 00 05 00 00 00 00 00  $FC 

;******************************************************************************************************************************************************

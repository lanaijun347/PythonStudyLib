
    车型ID：1008

    模拟：协议模拟-->1008

;******************************************************************************************************************************************************

    通讯线: $~06$~14$~500k$~72F

进入命令:

  $~  REQ000:727 02 10 01 00 00 00 00 00       $~  ANS000:72F 02 10 01 00 00 00 00 00
  $~  REQ001:727 02 10 01 00 00 00 00 00       $~  ANS001:72F 02 10 01 00 00 00 00 00

空闲命令:

  $!  REQ000:727 02 3E 80 00 00 00 00 00       $!  ANS000:72F 02 3E 80 00 00 00 00 00

退出命令:



;******************************************************************************************************************************************************

读取故障码:

  $#  REQ000:727 04 18 00 00 FF 00 00 00       $#  ANS000:72F 04 18 00 00 FF 00 00 00

		控制公式：
			y=(x1<<8)+x2;

  ANS000.BYTE02 控制故障码个数，从$#ANS000.BYTE3$#开始每$#3$#个字节表示一个故障码，前$#2$#表示码号。

  码库:

  $*$*DTC/feff0000_2BYTE


<RDTC>
  <FUNCTION type="0" num="0">
     <param type="string" value="1"/>
   </FUNCTION>
</RDTC>

;******************************************************************************************************************************************************

清除故障码:

  $$  REQ000:727 03 14 00 FF 00 00 00 00       $$  ANS000:72F 03 14 00 FF 00 00 00 00

;******************************************************************************************************************************************************

版本信息:

  $%  REQ000:727 03 22 00 01 00 00 00 00       $%  ANS000:72F 03 22 00 01 00 00 00 00

  $%  000:软件版本:        $%    $%  ANS000.BYTE004  $%  y = x1*0.1;
  $%  001:硬件版本:        $%    $%  ANS000.BYTE005  $%  y = x1*0.1;
  $%  002:版本日期:        $%    $%  ANS000.BYTE007  $%  y=SPRINTF([20%02d%s%02d%s%02d%s],x1,@0197,x2,@0198,x3,@0199);

;******************************************************************************************************************************************************

读数据流: 

  $  REQ000:727 03 22 00 04 00 00 00 00       $  ANS000:72F 03 22 00 04 00 00 00 00
  $  REQ001:727 03 22 00 05 00 00 00 00       $  ANS001:72F 03 22 00 05 00 00 00 00
  $  REQ002:727 03 22 00 06 00 00 00 00       $  ANS002:72F 03 22 00 06 00 00 00 00
  $  REQ003:727 03 22 00 07 00 00 00 00       $  ANS003:72F 03 22 00 07 00 00 00 00
  $  REQ004:727 03 22 00 0A 00 00 00 00       $  ANS004:72F 03 22 00 0A 00 00 00 00
  $  REQ005:727 03 22 00 0B 00 00 00 00       $  ANS005:72F 03 22 00 0B 00 00 00 00
  $  REQ006:727 03 22 00 0C 00 00 00 00       $  ANS006:72F 03 22 00 0C 00 00 00 00
  $  REQ007:727 03 22 00 0E 00 00 00 00       $  ANS007:72F 03 22 00 0E 00 00 00 00
  $  REQ008:727 03 22 00 0F 00 00 00 00       $  ANS008:72F 03 22 00 0F 00 00 00 00

  $  000.发动机允许信号                 $    $               $    $  ANS000.BYTE004  $  if(x1==0x00) y=@0196; else if(x1==0x01) y=@0195; else y=@09b4;
  $  001.风速                           $    $               $    $  ANS001.BYTE004  $  y = x1;
  $  002.送风模式                       $    $               $    $  ANS002.BYTE004  $  if(x1==0x00) y=@006d;else if(x1==0x01) y=@006e; else if(x1==0x02) y=@006f; else if(x1==0x03) y=@0070; else if(x1==0x04) y=@0071; else if(x1==0x05) y=@0072; else y=@03e4;
  $  003.燃油压缩机状态                 $    $               $    $  ANS003.BYTE004  $  if(x1==0x00) y=@0843; else if(x1==0x01) y=@0524; else y=@09b3;
  $  004.蒸发器温度                     $    $  degree C     $    $  ANS004.BYTE004  $  if(x1<0x1A) y=x1;else if(x1==0xD0)y = @01e4; else if(x1==0xD8)y = @01e3; else y=@0001;
  $  005.AC(空调控制器)请求状态         $    $               $    $  ANS005.BYTE004  $  if(x1==0x00) y=@0044; else if(x1==0x01) y=@003f; else y=@09b6;
  $  006.压力状态                       $    $               $    $  ANS006.BYTE004  $  if(x1==0x00) y=@0078; else if(x1==0x01) y=@01d8; else y=@09b5;
  $  007.压力值                         $    $  Mpa          $    $  ANS006.BYTE005  $  y = (x1*3+x2*653)/255;
  $  008.鼓风机正端继电器控制状态       $    $               $    $  ANS007.BYTE004  $  if(x1==0x00) y=@002a; else if(x1==0x01) y=@002b; else y=@0958;
  $  009.后除霜继电器控制状态           $    $               $    $  ANS008.BYTE004  $  if(x1==0x00) y=@002a; else if(x1==0x01) y=@002b; else y=@0958;

;******************************************************************************************************************************************************

30帧:

  $FC REQ000:727 30 00 05 00 00 00 00 00  $FC 

;******************************************************************************************************************************************************

...
进入命令:
  ...
  $~  REQ001:715 02 10 01 00 00 00 00 00       $~  ANS001:71D 02 10 01 00 00 00 00 00
...
读数据流: 
  ...
  $  000.转向角度                            $    $  °       $    $  ANS000.BYTE004  $  if((((x1<<8)+x2)>=0)&&(((x1<<8)+x2)<=32767))y=(((x1<<8)+x2))*1.0/10;else if((((x1<<8)+x2)>=32768)&&(((x1<<8)+x2)<=65535))y=((((x1<<8)+x2))-65536)*1.0/10;else y=@0001;
  $  001.转向角速度                          $    $  °/s     $    $  ANS001.BYTE004  $  if((((x1<<8)+x2)>=0)&&(((x1<<8)+x2)<=32767))y=(((x1<<8)+x2))*4;else if((((x1<<8)+x2)>=32768)&&(((x1<<8)+x2)<=65535))y=(32768-(((x1<<8)+x2)))*4;else y=@0001;

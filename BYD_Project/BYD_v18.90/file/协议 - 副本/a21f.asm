...
版本信息:
  ...
  $%  000:软件号:                 $%    $%  ANS000.BYTE004  $%  y=SPRINTF([%02X.%02X.%02X %d%s%d%s%d%s],x1,x2,x3,x4,@0197,x5,@0198,x6,@0199);
  $%  001:硬件号:                 $%    $%  ANS001.BYTE004  $%  y=SPRINTF([%02X.%02X.%02X %d%s%d%s%d%s],x1,x2,x3,x4,@0197,x5,@0198,x6,@0199);
...
读数据流: 
  ...
  $  000.读卡器供电电压       $    $       $    $  ANS000.BYTE004  $  if(x1==0x00) y=@0078;else if(x1==0xFF)y=@00cc;else y=@0525;
  $  001.读卡器通讯情况       $    $       $    $  ANS000.BYTE005  $  if(x1==0x00) y=@0526;else if(x1==0xFF)y=@0b44;else y=@0525;
  $  002.读卡器内部天线       $    $       $    $  ANS000.BYTE006  $  if(x1==0x00) y=@0078;else if(x1==0xFF)y=@0b43;else y=@0525;
  $  003.是否是本车钥匙       $    $       $    $  ANS000.BYTE007  $  if(x1==0x02) y=@0185;else if(x1==0x03)y=@0186; else y=@00cf;
  $  004.钥匙ID信息           $    $       $    $  ANS000.BYTE007  $  if(x1==0x02||x1==03)y=x2*16777216+x3*65536+x4*256+x5;else y=@0525;

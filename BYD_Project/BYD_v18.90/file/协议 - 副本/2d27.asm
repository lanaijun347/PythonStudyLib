...
读数据流: 
  ...
  $  REQ000:766 03 22 00 02 00 00 00 00       $  ANS000:76E 03 22 00 02 00 00 00 00
  ...
  $  REQ003:766 03 22 00 08 00 00 00 00       $  ANS003:76E 03 22 00 08 00 00 00 00
  $  REQ004:766 03 22 00 10 00 00 00 00       $  ANS004:76E 03 22 00 10 00 00 00 00
  $  REQ005:766 03 22 00 11 00 00 00 00       $  ANS005:76E 03 22 00 11 00 00 00 00
  $  REQ006:766 03 22 F1 8C 00 00 00 00       $  ANS006:76E 03 22 F1 8C 00 00 00 00

  $  000.生产日期                      $    $         $    $  ANS000.BYTE004  $  y=SPRINTF([%02d%s%02d%s%02d%s],x1,@0197,x2,@0198,x3,@0199);
  $  001.PTC(辅助加热)预置档位         $    $         $    $  ANS001.BYTE004  $  y=x1;
  $  002.PTC(辅助加热)实际档位         $    $         $    $  ANS002.BYTE004  $  y=x1;
  $  003.冷却液温度                    $    $  ℃     $    $  ANS003.BYTE004  $  if((x1>=0)&&(x1<=170)) y=x1;else if(x1==208) y=@0284;else if(x1==216) y=@0285;else y=@0001;
  $  004.12V低压电源值                 $    $  V      $    $  ANS004.BYTE004  $  y=SPRINTF([%.1f],(x1*256+x2)*0.1);
  $  005.负载高压电源                  $    $  V      $    $  ANS005.BYTE004  $  y=(x1*256+x2);
  $  006.PTC(辅助加热)产品序列号       $    $         $    $  ANS006.BYTE004  $  HEX(x1,x2,x3);
  $  007.生产日期                      $    $         $    $  ANS006.BYTE007  $  y=SPRINTF([%02d%s%02d%s%02d%s],x1,@0197,x2,@0198,x3,@0199);

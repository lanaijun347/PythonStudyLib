...
读数据流: 
  ...
  $  008.阀继电器状态            $    $            $    $  ANS002.BYTE004  $  if(x1&0x200) y=@001d;else y=@001e;
  ...
  $  020.速度限制监视状态        $    $            $    $  ANS005.BYTE004  $  switxh(x1)0x00: y=@0209;0x01: y=@020a;default: y=@0157;
  $  021.车速                    $    $            $    $  ANS006.BYTE004  $  y=SPRINTF([%d],(x1*256+x2)*0.06875);
  $  022.主缸压力值              $    $  bar       $    $  ANS007.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],(x1*256+x2)*0.0153);else y=SPRINTF([%.1f],((x1-256)*256+x2)*0.0153);
  $  023.转角传感器              $    $  °        $    $  ANS007.BYTE006  $  if(x1<0x80) y=SPRINTF([%.2f],(x1*256+x2)*0.1);else y=SPRINTF([%.2f],((x1-256)*256+x2)*0.1);
  $  024.偏航率传感器            $    $  rad/s     $    $  ANS007.BYTE008  $  if(x1<0x80) y=SPRINTF([%.3f],(x1*256+x2)*0.00213);else y=SPRINTF([%.3f],((x1-256)*256+x2)*0.00213);
  $  025.横向加速度传感器        $    $  m/s^2     $    $  ANS007.BYTE010  $  if(x1<0x80) y=SPRINTF([%.3f],(x1*256+x2)*0.02712);else y=SPRINTF([%.3f],((x1-256)*256+x2)*0.02712);
  $  026.纵向加速度传感器        $    $  m/s^2     $    $  ANS007.BYTE012  $  if(x1<0x80) y=SPRINTF([%.3f],(x1*256+x2)*0.02712);else y=SPRINTF([%.3f],((x1-256)*256+x2)*0.02712);

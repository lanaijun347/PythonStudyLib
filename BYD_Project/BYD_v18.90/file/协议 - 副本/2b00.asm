...
读数据流: 
  ...
  $  022.加注状态                        $    $            $    $  ANS003.BYTE004  $  switxh(x1)0x00: y=@0206;0xAA: y=@0207;0xEE: y=@0208;default: y=@007b;
  $  023.下线检测状态                    $    $            $    $  ANS004.BYTE004  $  switxh(x1)0x00: y=@0203;0xAA: y=@0204;0xEE: y=@0205;default: y=@0157;
  $  024.速度限制监视器                  $    $            $    $  ANS005.BYTE004  $  switxh(x1)0x00: y=@0209;0x01: y=@020a;default: y=@0157;
  $  025.EPB(电子驻车系统)变量配置       $    $            $    $  ANS005.BYTE004  $  switxh(x1)0xBB: y=@098a;0xDD: y=@098b;default: y=@0001;
  $  026.车速                            $    $            $    $  ANS006.BYTE004  $  y=SPRINTF([%d],(x1*256+x2)*0.06875);
  $  027.主缸压力值                      $    $  bar       $    $  ANS007.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],(x1*256+x2)*0.0153);else y=SPRINTF([%.1f],((x1-256)*256+x2)*0.0153);
  $  028.转角传感器                      $    $  °        $    $  ANS007.BYTE006  $  if(x1<0x80) y=SPRINTF([%.2f],(x1*256+x2)*0.1);else y=SPRINTF([%.2f],((x1-256)*256+x2)*0.1);
  $  029.偏航率传感器                    $    $  rad/s     $    $  ANS007.BYTE008  $  if(x1<0x80) y=SPRINTF([%.3f],(x1*256+x2)*0.00213);else y=SPRINTF([%.3f],((x1-256)*256+x2)*0.00213);
  $  030.横向加速度传感器                $    $  m/s^2     $    $  ANS007.BYTE010  $  if(x1<0x80) y=SPRINTF([%.3f],(x1*256+x2)*0.02712);else y=SPRINTF([%.3f],((x1-256)*256+x2)*0.02712);
  $  031.纵向加速度传感器                $    $  m/s^2     $    $  ANS007.BYTE012  $  if(x1<0x80) y=SPRINTF([%.3f],(x1*256+x2)*0.02712);else y=SPRINTF([%.3f],((x1-256)*256+x2)*0.02712);

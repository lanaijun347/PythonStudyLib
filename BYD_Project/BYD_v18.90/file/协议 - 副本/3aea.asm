...
读数据流: 
  ...
  $  003.记忆倒车翻转前垂直位置电压         $    $  V     $    $  ANS001.BYTE004  $  y=SPRINTF([%.2f],(x1*256+x2)*0.01);
  $  004.记忆倒车翻转前水平位置电压         $    $  V     $    $  ANS001.BYTE006  $  y=SPRINTF([%.2f],(x1*256+x2)*0.01);
  $  005.后视镜垂直传感器电压               $    $  V     $    $  ANS001.BYTE008  $  y=SPRINTF([%.2f],(x1*256+x2)*0.01);
  $  006.后视镜水平传感器电压               $    $  V     $    $  ANS001.BYTE010  $  y=SPRINTF([%.2f],(x1*256+x2)*0.01);
  $  007.右外后视镜折叠电机                 $    $        $    $  ANS001.BYTE012  $  if(((x1*256+x2)>=0)&&((x1*256+x2)<=25000)) y=(x1*256+x2);else if((x1*256+x2)==65535) y=@0000;else y=@0001;
  $  008.右外后视镜换向器水平位置电机       $    $        $    $  ANS001.BYTE014  $  if(((x1*256+x2)>=0)&&((x1*256+x2)<=20000)) y=(x1*256+x2);else if((x1*256+x2)==65535) y=@0000;else y=@0001;
  $  009.右外后视镜换向器垂直位置电机       $    $        $    $  ANS001.BYTE016  $  if(((x1*256+x2)>=0)&&((x1*256+x2)<=20000)) y=(x1*256+x2);else if((x1*256+x2)==65535) y=@0000;else y=@0001;
  $  010.右外后视镜照脚灯点亮时间           $    $  H     $    $  ANS001.BYTE018  $  if(((x1*256+x2)>=0)&&((x1*256+x2)<=1000)) y=(x1*256+x2);else if((x1*256+x2)==4095) y=@0000;else y=@0001;
  $  011.右外后视镜加热时间                 $    $  H     $    $  ANS001.BYTE020  $  if(((x1*256+x2)>=0)&&((x1*256+x2)<=500)) y=(x1*256+x2);else if((x1*256+x2)==4095) y=@0000;else y=@0001;

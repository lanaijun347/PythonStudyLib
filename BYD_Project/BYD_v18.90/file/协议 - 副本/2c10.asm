...
读数据流: 
  ...
  $  026.车速                            $    $  km/h      $    $  ANS007.BYTE004  $  y=SPRINTF([%.5f],(x1*256+x2)*0.06875);
  ...
  $  032.主缸压力                        $    $  bar       $    $  ANS008.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],(x1*256+x2)*0.0153);else y=SPRINTF([%.1f],((x1-256)*256+x2)*0.0153);
  $  033.转向角                          $    $  °        $    $  ANS008.BYTE006  $  if(x1<0x80) y=SPRINTF([%.1f],(x1*256+x2)*0.1);else y=SPRINTF([%.1f],((x1-256)*256+x2)*0.1);
  $  034.偏航率                          $    $  rad/s     $    $  ANS008.BYTE008  $  if(x1<0x80) y=SPRINTF([%.2f],(x1*256+x2)*0.00213);else y=SPRINTF([%.2f],((x1-256)*256+x2)*0.00213);
  $  035.横向加速度                      $    $  m/s^2     $    $  ANS008.BYTE010  $  if(x1<0x80) y=SPRINTF([%.2f],(x1*256+x2)*0.02712);else y=SPRINTF([%.2f],((x1-256)*256+x2)*0.02712);
  $  036.纵向加速度                      $    $  m/s^2     $    $  ANS008.BYTE012  $  if(x1<0x80) y=SPRINTF([%.2f],(x1*256+x2)*0.02712);else y=SPRINTF([%.2f],((x1-256)*256+x2)*0.02712);

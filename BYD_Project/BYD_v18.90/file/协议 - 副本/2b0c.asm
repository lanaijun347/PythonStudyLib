...
读数据流: 
  ...
  $  017.无极风扇请求状态                  $    $  %        $    $  ANS017.BYTE004  $  if((x1>=0)&&(x1<=100)) y=x1;else if(x1==255) y=@007b;else y=@0001;
  $  018.真空泵状态                        $    $           $    $  ANS018.BYTE004  $  if(x1==0) y=@0238;else if(x1==1) y=@0239;else y=@0001;
  $  019.风扇高速继电器状态                $    $           $    $  ANS019.BYTE004  $  if(x1==0) y=@0236;else if(x1==1) y=@0237;else if(x1==255) y=@007b;else y=@0001;
  $  020.风扇低速继电器状态                $    $           $    $  ANS020.BYTE004  $  if(x1==0) y=@0233;else if(x1==1) y=@0234;else if(x1==255) y=@007b;else y=@0001;
  $  021.EPB(电子驻车系统)状态             $    $           $    $  ANS021.BYTE004  $  if((x2*256+x1)==0) y=@0102;else if((x2*256+x1)==1) y=@0117;else if((x2*256+x1)==2) y=@0232;else if((x2*256+x1)==3) y=@01ef;else if((x2*256+x1)==4) y=@0167;else y=@0001;
  $  022.坡道坡度                          $    $           $    $  ANS022.BYTE004  $  y=SPRINTF([%d],(x2*256+x1)-100);
  $  023.售后软件版本号                    $    $           $    $  ANS023.BYTE004  $  y=SPRINTF([%d.%d.%d],x1,x2,x3);
  $  024.水温值                            $    $  ℃       $    $  ANS024.BYTE004  $  if((x1>=0)&&(x1<=200)) y=x1-40;else if(x1==255) y=@007b;else y=@0001;
  $  025.真空压力值                        $    $  kpa      $    $  ANS025.BYTE004  $  y=x1;
  $  026.水温报警                          $    $           $    $  ANS026.BYTE004  $  if(x1==0) y=@023f;else if(x1==1) y=@0240;else if(x1==255) y=@007b;else y=@0001;
  ...
  $  030.整车车速                          $    $  km/h     $    $  ANS030.BYTE004  $  if(((x2*256+x1)>=0)&&((x2*256+x1)<=3000)) y=SPRINTF([%.1f],(x2*256+x1)*0.1);else if((x2*256+x1)==65535) y=@0027;else y=@007b;
  $  031.总里程                            $    $  km       $    $  ANS031.BYTE004  $  y=SPRINTF([%.1f],(x3*256*256+x2*256+x1)/10);
  $  032.倾角标定工作信息/是否校准         $    $           $    $  ANS032.BYTE004  $  if(x1==0) y=@015d;else if(x1==1) y=@015e;else y=@0120;
  ...
  $  037.读取巡航配置状态                  $    $           $    $  ANS037.BYTE004  $  if((x2*256+x1)==0) y=@0157;else if((x2*256+x1)==1) y=@022e;else if((x2*256+x1)==2) y=@022f;else y=@0120;
  $  038.读取制动开关状态                  $    $           $    $  ANS038.BYTE004  $  if((x2*256+x1)==0) y=@0182;else if((x2*256+x1)==1) y=@0118;else if((x2*256+x1)==2) y=@00d8;else y=@0001;
  $  039.读取VIN(车辆识别码)是否写入       $    $           $    $  ANS039.BYTE004  $  if((x2*256+x1)==0) y=@0230;else if((x2*256+x1)==1) y=@0231;else y=@0001;

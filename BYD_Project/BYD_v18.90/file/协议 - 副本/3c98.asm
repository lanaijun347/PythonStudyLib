
    车型ID：3c98

    模拟：协议模拟-->3c98

;******************************************************************************************************************************************************

    通讯线: $~06$~14$~500k$~78B

进入命令:

  $~  REQ000:783 02 10 01 00 00 00 00 00       $~  ANS000:78B 02 10 01 00 00 00 00 00

空闲命令:

  $!  REQ000:783 02 3E 80 00 00 00 00 00       $!  ANS000:78B 02 3E 80 00 00 00 00 00

退出命令:



;******************************************************************************************************************************************************

读取故障码:

  $#  REQ000:783 03 19 02 09 00 00 00 00       $#  ANS000:78B 03 19 02 09 00 00 00 00

		控制公式：
			y=(x1<<16)+(x2<<8)+x3;

  帧长控制故障码个数，从$#ANS000.BYTE4$#开始每$#4$#个字节表示一个故障码，前$#3$#表示码号。

  码库:

  $*$*DTC/17000000


;******************************************************************************************************************************************************

清除故障码:

  $$  REQ000:783 04 14 FF FF FF 00 00 00       $$  ANS000:78B 04 14 FF FF FF 00 00 00

;******************************************************************************************************************************************************

版本信息:

  $%  REQ000:783 03 22 F1 95 00 00 00 00       $%  ANS000:78B 03 22 F1 95 00 00 00 00
  $%  REQ001:783 03 22 F1 94 00 00 00 00       $%  ANS001:78B 03 22 F1 94 00 00 00 00
  $%  REQ002:783 03 22 F1 93 00 00 00 00       $%  ANS002:78B 03 22 F1 93 00 00 00 00
  $%  REQ003:783 03 22 F1 80 00 00 00 00       $%  ANS003:78B 03 22 F1 80 00 00 00 00

  $%  000:软件版本:            $%    $%  ANS000.BYTE004  $%  y=SPRINTF([%d.%d%d.%d%d],((x1*256+x2)%100000-(x1*256+x2)%10000)/10000,((x1*256+x2)%10000-(x1*256+x2)%1000)/1000,((x1*256+x2)%1000-(x1*256+x2)%100)/100,((x1*256+x2)%100-(x1*256+x2)%10)/10,(x1*256+x2)%10);
  $%  001:软件日期:            $%    $%  ANS000.BYTE006  $%  y=SPRINTF([%02d%s%02d%s%02d%s],x1,@0197,x2,@0198,x3,@0199);
  $%  002:修改次数:            $%    $%  ANS000.BYTE009  $%  y=x1;
  $%  003:软件编号:            $%    $%  ANS001.BYTE004  $%  y=SPRINTF([%02X%02X%02X%02X%02X%02X%02X%02X%02X],x1,x2,x3,x4,x5,x6,x7,x8,x9);
  $%  004:硬件版本:            $%    $%  ANS002.BYTE004  $%  y=SPRINTF([%d.%d.%d],(x1%1000-x1%100)/100,(x1%100-x1%10)/10,x1%10);
  $%  005:硬件日期:            $%    $%  ANS002.BYTE005  $%  y=SPRINTF([%02d%s%02d%s%02d%s],x1,@0197,x2,@0198,x3,@0199);
  $%  006:boot版本号:          $%    $%  ANS003.BYTE004  $%  y=SPRINTF([%d%s%d%s%s%d],x1+2000,@0197,x2,@019a,@019b,x3);
  $%  007:boot版本信息:        $%    $%  ANS003.BYTE007  $%  y=SPRINTF([%d%d%d],x1,x2,x3);
  $%  008:发布日期:            $%    $%  ANS003.BYTE010  $%  y=SPRINTF([%02d%s%02d%s%02d%s],x1,@0197,x2,@0198,x3,@0199);

;******************************************************************************************************************************************************

读数据流: 

  $  REQ000:783 03 22 00 01 00 00 00 00       $  ANS000:78B 03 22 00 01 00 00 00 00
  $  REQ001:783 03 22 00 02 00 00 00 00       $  ANS001:78B 03 22 00 02 00 00 00 00
  $  REQ002:783 03 22 00 03 00 00 00 00       $  ANS002:78B 03 22 00 03 00 00 00 00
  $  REQ003:783 03 22 00 04 00 00 00 00       $  ANS003:78B 03 22 00 04 00 00 00 00
  $  REQ004:783 03 22 00 05 00 00 00 00       $  ANS004:78B 03 22 00 05 00 00 00 00
  $  REQ005:783 03 22 00 06 00 00 00 00       $  ANS005:78B 03 22 00 06 00 00 00 00
  $  REQ006:783 03 22 00 07 00 00 00 00       $  ANS006:78B 03 22 00 07 00 00 00 00
  $  REQ007:783 03 22 00 08 00 00 00 00       $  ANS007:78B 03 22 00 08 00 00 00 00
  $  REQ008:783 03 22 00 09 00 00 00 00       $  ANS008:78B 03 22 00 09 00 00 00 00
  $  REQ009:783 03 22 00 0A 00 00 00 00       $  ANS009:78B 03 22 00 0A 00 00 00 00
  $  REQ010:783 03 22 00 0B 00 00 00 00       $  ANS010:78B 03 22 00 0B 00 00 00 00
  $  REQ011:783 03 22 00 0C 00 00 00 00       $  ANS011:78B 03 22 00 0C 00 00 00 00

  $  000.ECU供电电压              $    $  V          $    $  ANS000.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],x1/10);else y=SPRINTF([%.1f],(x1-256)/10);
  $  001.车速                     $    $  Km/h       $    $  ANS001.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],(x1*256+x2)*0.06875);else y=SPRINTF([%d],((x1-256)*256+x2)*0.06875);
  $  002.转向角                   $    $  degree     $    $  ANS002.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],(x1*256+x2)-740);else y=SPRINTF([%d],((x1-256)*256+x2)-740);
  $  003.ECU温度                  $    $  ℃         $    $  ANS003.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],x1-40);else y=SPRINTF([%d],(x1-256)-40);
  $  004.电机电流                 $    $  A          $    $  ANS004.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],(x1*256+x2)*0.1);else y=SPRINTF([%d],((x1-256)*256+x2)*0.1);
  $  005.方向盘扭矩               $    $  Nm         $    $  ANS005.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],(x1*256+x2)*0.01);else y=SPRINTF([%d],((x1-256)*256+x2)*0.01);
  $  006.扭矩传感器供电电压       $    $  V          $    $  ANS006.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],x1*0.1);else y=SPRINTF([%d],(x1-256)*0.1);
  $  007.助力模式                 $    $             $    $  ANS007.BYTE004  $  if(x1==0) y=@02fc;else if(x1==1) y=@02fd;else if(x1==2) y=@02fe;else y=@0001;
  $  008.端部保护状态             $    $             $    $  ANS008.BYTE004  $  if(x1==0) y=@02f0;else if(x1==1) y=@02f1;else y=@0001;
  $  009.电机保护状态             $    $             $    $  ANS009.BYTE004  $  if(x1==0) y=@02f0;else if(x1==1) y=@083b;else y=@0001;
  $  010.电机转速                 $    $  RPM        $    $  ANS010.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],(x1*256+x2));else y=SPRINTF([%d],((x1-256)*256+x2));
  $  011.转向角速度               $    $  °/s       $    $  ANS011.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],(x1*256+x2));else y=SPRINTF([%d],((x1-256)*256+x2));

;******************************************************************************************************************************************************

30帧:

  $FC REQ000:783 30 01 05 00 00 00 00 00  $FC 

;******************************************************************************************************************************************************

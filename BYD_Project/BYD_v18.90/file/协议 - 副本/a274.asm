
    车型ID：a274

    模拟：协议模拟-->a274

;******************************************************************************************************************************************************

    通讯线: $~06$~14$~500k$~73A

进入命令:

  $~  REQ000:732 02 10 01 00 00 00 00 00       $~  ANS000:73A 02 10 01 00 00 00 00 00

空闲命令:

  $!  REQ000:732 02 3E 01 00 00 00 00 00       $!  ANS000:73A 02 3E 01 00 00 00 00 00

退出命令:



;******************************************************************************************************************************************************

读取故障码:

  $#  REQ000:732 04 18 00 00 FF 00 00 00       $#  ANS000:73A 04 18 00 00 FF 00 00 00

		控制公式：
			y=(x1<<8)+x2;

  ANS000.BYTE02 控制故障码个数，从$#ANS000.BYTE3$#开始每$#3$#个字节表示一个故障码，前$#2$#表示码号。

  码库:

  $*$*DTC/a2740000_2BYTE


<RDTC>
  <FUNCTION type="0" num="0">
     <param type="string" value="1"/>
   </FUNCTION>
</RDTC>

;******************************************************************************************************************************************************

清除故障码:

  $$  REQ000:732 03 14 00 FF 00 00 00 00       $$  ANS000:73A 03 14 00 FF 00 00 00 00

;******************************************************************************************************************************************************

版本信息:

  $%  REQ000:732 03 22 00 01 00 00 00 00       $%  ANS000:73A 03 22 00 01 00 00 00 00

  $%  000:ECU车型系列:        $%    $%  ANS000.BYTE004  $%  HEX(x1,x2,x3,x4,x5,x6,x7,x8);
  $%  001:驱动数:             $%    $%  ANS000.BYTE005  $%  y=x1;
  $%  002:软件版本:           $%    $%  ANS000.BYTE007  $%  y=x1/10.0;
  $%  003:硬件版本:           $%    $%  ANS000.BYTE008  $%  y=x1/10.0;

;******************************************************************************************************************************************************

读数据流: 

  $  REQ000:732 03 22 00 02 00 00 00 00       $  ANS000:73A 03 22 00 02 00 00 00 00
  $  REQ001:732 03 22 00 0D 00 00 00 00       $  ANS001:73A 03 22 00 0D 00 00 00 00
  $  REQ002:732 03 22 00 05 00 00 00 00       $  ANS002:73A 03 22 00 05 00 00 00 00

  $  000.车速           $    $  km/h     $    $  ANS000.BYTE027  $  y=x1;
  $  001.加密信息       $    $           $    $  ANS001.BYTE004  $  if(x1==0x00) y=@02f5;else if(x1==0x01) y=@02f6;else y=@0525;
  $  002.访问次数       $    $           $    $  ANS002.BYTE010  $  y=x1;

;******************************************************************************************************************************************************

30帧:

  $FC REQ000:732 30 00 00 00 00 00 00 00  $FC 

;******************************************************************************************************************************************************

...
读数据流: 
  ...
  $  000.散热片温度                    $    $  degree C     $    $  ANS000.BYTE004  $  if(x>0xAA) y=@006d;else y=x;
  $  001.PTC(辅助加热)预置档位         $    $               $    $  ANS001.BYTE004  $  if(x>0x64) y=@006d;else y=x;
  $  002.PTC(辅助加热)实际档位         $    $               $    $  ANS002.BYTE004  $  if(x>0x64) y=@006d;else y=x;
  $  003.右侧散热片温度                $    $  degree C     $    $  ANS003.BYTE004  $  if(x>0xAA) y=@006d;else y=x;
  $  004.冷却液温度                    $    $  degree C     $    $  ANS004.BYTE004  $  if(x>0x3C) y=@006d;else y=2*x;
  $  005.IG2(点火2)低压电源值          $    $  V            $    $  ANS005.BYTE004  $  if((x1>0x00)||((x1==0x00)&&(x2>0xf0))) y=@006d;else y=0.1*(x1*256+x2);
  $  006.负载高压电源值                $    $  V            $    $  ANS006.BYTE004  $  if((x1>0x01)||((x1==0x01)&&(x2>0xf4))) y=@006d;else y=x1*256+x2;
  $  007.负载电流值                    $    $  A            $    $  ANS007.BYTE004  $  if((x1>0x01)||((x1==0x01)&&(x2>0x2C))) y=@006d;else y=0.1*(x1*256+x2);
  $  008.PTC(辅助加热)消耗功率         $    $  W            $    $  ANS008.BYTE004  $  if((x1>0x17)||((x1==0x17)&&(x2>0x70))) y=@006d;else y=x1*256+x2;
  $  009.PTC(辅助加热)产品序列号       $    $               $    $  ANS009.BYTE004  $  if(x1<27) y=SPRINTF([%02d/%02d/%02d],x1,x2,x3);else y=@006d;
  $  010.生产日期                      $    $               $    $  ANS009.BYTE007  $  y=SPRINTF([20%02d%s%02d%s%02d%s],x1,@0197,x2,@0198,x3,@0199);

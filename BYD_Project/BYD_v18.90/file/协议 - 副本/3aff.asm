...
读数据流: 
  ...
  $  REQ016:780 03 22 00 16 00 00 00 00       $  ANS016:788 03 22 00 16 00 00 00 00
  $  REQ017:780 03 22 00 17 00 00 00 00       $  ANS017:788 03 22 00 17 00 00 00 00
  $  REQ018:780 03 22 00 19 00 00 00 00       $  ANS018:788 03 22 00 19 00 00 00 00
  ...
  $  REQ023:780 03 22 00 24 00 00 00 00       $  ANS023:788 03 22 00 24 00 00 00 00
  ...
  $  001.前驱动电机控制器启动允许                           $    $           $    $  ANS001.BYTE004  $  switxh((x2*256+x1))0x00: y=@013f;0x01: y=@0178;default: y=@0001;
  ...
  $  004.前驱动电机转速                                     $    $  rpm      $    $  ANS004.BYTE004  $  y=SPRINTF([%d],(x2*256+x1)-18000);
  $  005.前驱动电机扭矩                                     $    $  N·M     $    $  ANS005.BYTE004  $  y=SPRINTF([%.1f],(x2*256+x1)*0.1-500);
  $  006.前驱动电机功率                                     $    $  kw       $    $  ANS006.BYTE004  $  y=SPRINTF([%.1f],(x2*256+x1)*0.1-100);
  $  007.冷却液温度                                         $    $  ℃       $    $  ANS007.BYTE004  $  if(((x2*256+x1)>=0)&&((x2*256+x1)<=200)) y=(x2*256+x1)-40;else if((x2*256+x1)==255) y=@02f8;else y=@0001;
  $  008.前驱动电机控制器IPM(智能功率模块)散热器温度        $    $  ℃       $    $  ANS008.BYTE004  $  if(((x2*256+x1)>=0)&&((x2*256+x1)<=200)) y=(x2*256+x1)-40;else if((x2*256+x1)==255) y=@0224;else y=@0001;
  $  009.前驱动电机温度                                     $    $  ℃       $    $  ANS009.BYTE004  $  y=SPRINTF([%d],(x2*256+x1)-40);
  $  010.前驱动电机控制器IGBT(绝缘栅双极型晶体管)温度       $    $  ℃       $    $  ANS010.BYTE004  $  y=SPRINTF([%d],(x2*256+x1)-40);
  $  011.前驱动电机A相电流                                  $    $  A        $    $  ANS011.BYTE004  $  y=(x2*256+x1);
  $  012.前驱动电机B相电流                                  $    $  A        $    $  ANS012.BYTE004  $  y=(x2*256+x1);
  $  013.前驱动电机C相电流                                  $    $  A        $    $  ANS013.BYTE004  $  y=(x2*256+x1);
  $  014.前驱动电机过载系数                                 $    $  %        $    $  ANS014.BYTE004  $  y=(x2*256+x1);
  $  015.售后软件版本号                                     $    $           $    $  ANS015.BYTE004  $  y=SPRINTF([%d.%d.%d],x1,x2,x3);
  $  016.前驱动电机旋变状态                                 $    $           $    $  ANS016.BYTE004  $  switxh((x2*256+x1))0x00: y=@0078;0x01: y=@0219;0x02: y=@021a;0x03: y=@021b;default: y=@0001;
  $  017.前驱过流状态                                       $    $           $    $  ANS017.BYTE004  $  switxh((x2*256+x1))0x00: y=@0078;0x01: y=@0167;default: y=@0001;
  $  018.前驱动电机控制器IPM(智能功率模块)状态              $    $           $    $  ANS018.BYTE004  $  switxh((x2*256+x1))0x00: y=@0078;0x01: y=@0167;default: y=@0001;
  $  019.电机零位值                                         $    $           $    $  ANS019.BYTE004  $  HEX(x2,x1);
  $  020.电机零位标定状态                                   $    $           $    $  ANS020.BYTE004  $  if(x1==0) y=@0157;else if(x1==1) y=@0221;else if(x1==2) y=@0222;else y=@00d8;
  ...
  $  024.主动泄放状态                                       $    $           $    $  ANS023.BYTE004  $  if(x1==0) y=@0215;else if(x1==1) y=@0216;else if(x1==2) y=@0217;else if(x1==3) y=@0218;else y=@0202;

...
读数据流: 
  ...
  $  000.方向盘转速                          $    $  deg/s     $    $  ANS000.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],x1*0.1);else y=SPRINTF([%.1f],(x1-256)*0.1);
  $  001.EPS(电动助力转向系统)相对转角       $    $  deg       $    $  ANS001.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],x1);else y=SPRINTF([%d],(x1-256));
  $  002.方向盘转角                          $    $  deg       $    $  ANS002.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],x1*0.1);else y=SPRINTF([%.1f],(x1-256)*0.1);
  $  003.方向盘力矩                          $    $  Nm        $    $  ANS003.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],x1*0.1);else y=SPRINTF([%.1f],(x1-256)*0.1);
  $  004.电机温度                            $    $  ℃        $    $  ANS004.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],x1*2);else y=SPRINTF([%d],(x1-256)*2);
  $  005.ECU内部温度                         $    $  ℃        $    $  ANS005.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],x1*2);else y=SPRINTF([%d],(x1-256)*2);
  $  006.电机电流                            $    $  A         $    $  ANS006.BYTE004  $  if(x1<0x80) y=SPRINTF([%d],x1);else y=SPRINTF([%d],(x1-256));
  $  007.供电电压                            $    $  V         $    $  ANS007.BYTE004  $  y=SPRINTF([%.4f],x1*0.003906);
  $  008.电机转速                            $    $  deg/s     $    $  ANS008.BYTE004  $  if(x1<0x80) y=SPRINTF([%.2f],x1*2.8125);else y=SPRINTF([%.2f],(x1-256)*2.8125);
  $  009.车速                                $    $            $    $  ANS009.BYTE004  $  y=SPRINTF([%.2f],x1*0.01);
  $  010.末端保护使能                        $    $            $    $  ANS010.BYTE004  $  switxh(x1)0x00: y=@001e;0x01: y=@020b;default: y=@0001;
  $  011.末端保护左端初始位置                $    $            $    $  ANS011.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],x1/16);else y=SPRINTF([%.1f],(x1-256)/16);
  $  012.末端保护右端初始位置                $    $            $    $  ANS012.BYTE004  $  if(x1<0x80) y=SPRINTF([%.1f],x1/16);else y=SPRINTF([%.1f],(x1-256)/16);

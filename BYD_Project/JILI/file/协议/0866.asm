...
读数据流: 
  ...
  $  000.ECU(电子控制器)供电电压       $    $  V        $    $  ANS000.BYTE004  $  if(x==0xff) y=@0008; else y=x*0.1;
  $  001.转向角                        $    $  deg      $    $  ANS001.BYTE004  $  if((x1*0x100+x2)<=15599) y=(X1*256+x2) *0.1-780; else y=@0132;
  $  002.ECU(电子控制器)温度           $    $  degC     $    $  ANS002.BYTE004  $  if(x==0xff) y=@0008;else y=x-40;
  ...
  $  010.电池电流                      $    $  A        $    $  ANS009.BYTE004  $  if(x==0xff) y=@0008; else y=x*0.5;
  $  011.方向盘角度内部偏置            $    $  deg      $    $  ANS010.BYTE004  $  if(x==0xff) y=@0008;  else y=x-127;

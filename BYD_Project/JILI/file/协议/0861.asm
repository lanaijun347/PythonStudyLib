...
读数据流: 
  ...
  $  REQ012:7A0 03 22 68 10 00 00 00 00       $  ANS012:7A8 03 22 68 10 00 00 00 00
  ...
  $  REQ022:7A0 03 22 68 1B 00 00 00 00       $  ANS022:7A8 03 22 68 1B 00 00 00 00
  $  REQ023:7A0 03 22 68 1E 00 00 00 00       $  ANS023:7A8 03 22 68 1E 00 00 00 00
  $  REQ024:7A0 03 22 68 1F 00 00 00 00       $  ANS024:7A8 03 22 68 1F 00 00 00 00
  $  REQ025:7A0 03 22 68 20 00 00 00 00       $  ANS025:7A8 03 22 68 20 00 00 00 00
  $  REQ026:7A0 03 22 68 21 00 00 00 00       $  ANS026:7A8 03 22 68 21 00 00 00 00
  $  REQ027:7A0 03 22 68 22 00 00 00 00       $  ANS027:7A8 03 22 68 22 00 00 00 00
  ...
  $  REQ031:7A0 03 22 68 26 00 00 00 00       $  ANS031:7A8 03 22 68 26 00 00 00 00
  $  REQ032:7A0 03 22 68 27 00 00 00 00       $  ANS032:7A8 03 22 68 27 00 00 00 00
  $  REQ033:7A0 03 22 68 28 00 00 00 00       $  ANS033:7A8 03 22 68 28 00 00 00 00
  $  REQ034:7A0 03 22 68 2A 00 00 00 00       $  ANS034:7A8 03 22 68 2A 00 00 00 00
  $  REQ035:7A0 03 22 68 2B 00 00 00 00       $  ANS035:7A8 03 22 68 2B 00 00 00 00
  $  REQ036:7A0 03 22 68 2D 00 00 00 00       $  ANS036:7A8 03 22 68 2D 00 00 00 00
  $  REQ037:7A0 03 22 68 2E 00 00 00 00       $  ANS037:7A8 03 22 68 2E 00 00 00 00
  $  REQ038:7A0 03 22 68 31 00 00 00 00       $  ANS038:7A8 03 22 68 31 00 00 00 00
  $  REQ039:7A0 03 22 10 00 00 00 00 00       $  ANS039:7A8 03 22 10 00 00 00 00 00
  ...
  $  012.T-BOX(远程信息处理控制器)温度                                                                             $    $  degC      $    $  ANS012.BYTE004  $  y=x-48;
  ...
  $  016.Led背光灯状态                                                                                             $    $  /         $    $  ANS015.BYTE004  $  if(0x00==x)y=@0cdd;else if(0x01==x)y=@0cde;else y=@0162;
  ...
  $  023.IP地址类型                                                                                                $    $  /         $    $  ANS022.BYTE004  $  switxh(x) 0x00:y=@0539;0x01:y=@053a;default:y=@0009;
  $  024.IP地址                                                                                                    $    $  /         $    $  ANS022.BYTE004  $  switxh(((x1>>0)&1))0x00:y=SPRINTF([%d.%d.%d.%d],X2,x3,x4,x5);0x01:y=SPRINTF([%x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x],x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17);
  $  025.访问端口                                                                                                  $    $  /         $    $  ANS022.BYTE021  $  HEX(x1,x2);
  $  026.PKI密码写入状态                                                                                           $    $  /         $    $  ANS023.BYTE004  $  if(x==0x00)y=@0241;else if(x==0x01)y=@0242; else y=@062f;
  $  027.开始读取故障信息延时时间(车辆电源IG-ON持续XX秒数后开始故障读取的时间)                                     $    $  s         $    $  ANS024.BYTE004  $  y=x;
  $  028.故障信息定时上传间隔时间(前一次上传XX分钟数后再次开始故障读取的时间)                                      $    $  min       $    $  ANS025.BYTE004  $  y=x;
  $  029.等待网络唤醒时间(远程控制前需要唤醒整车网络等待超时的时间)                                                $    $  ms        $    $  ANS026.BYTE004  $  y=x*10;
  $  030.远程控制超时时间                                                                                          $    $  s         $    $  ANS027.BYTE004  $  y=x;
  ...
  $  034.短信发送延时时间(MQTT失败后转为发短信的延时时间)                                                          $    $  s         $    $  ANS031.BYTE004  $  y=x;
  $  035.日志存储最大时间(日志信息存储在T-BOX(远程信息处理控制器)的最大时间)                                       $    $  h         $    $  ANS032.BYTE004  $  y=x;
  $  036.服务器数据存储最大时间(数据存在在服务器的最大时间（因T-BOX(远程信息处理控制器)处于Sleep_Pool模式）)       $    $  h         $    $  ANS033.BYTE004  $  y=x;
  $  037.驾驶行为信息定时上传间隔时间                                                                              $    $  min       $    $  ANS034.BYTE004  $  y=x;
  $  038.远程控制结果上传延时时间（收到整车反馈的控制结果至上传车身数据的延时时间）                                $    $  ms        $    $  ANS035.BYTE004  $  y=x*10;
  $  039.ESK写入状态                                                                                               $    $  /         $    $  ANS036.BYTE004  $  switxh(x)0x00:y=@0536;0x01:y=@0f0e;default:y=@0162;
  $  040.睡眠时间标定                                                                                              $    $  s         $    $  ANS037.BYTE004  $  y=x1*256+x2;
  $  041.PKI写入状态                                                                                               $    $  /         $    $  ANS038.BYTE004  $  switxh(x)0x00:y=@0901;0x01:y=@09de;default:y=@0162;
  $  042.禁止远程控制                                                                                              $    $  /         $    $  ANS039.BYTE004  $  if(x==0x00)y=@10c0;else if(x==0x01)y=@10c1;

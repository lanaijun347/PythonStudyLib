...
读数据流: 
  ...
  $  REQ024:7E0 03 22 01 07 00 00 00 00       $  ANS024:7E8 03 22 01 07 00 00 00 00
  ...
  $  REQ041:7E0 03 22 09 01 00 00 00 00       $  ANS041:7E8 03 22 09 01 00 00 00 00
  $  REQ042:7E0 03 22 09 0E 00 00 00 00       $  ANS042:7E8 03 22 09 0E 00 00 00 00
  $  REQ043:7E0 03 22 01 01 00 00 00 00       $  ANS043:7E8 03 22 01 01 00 00 00 00
  $  REQ044:7E0 03 22 01 96 00 00 00 00       $  ANS044:7E8 03 22 01 96 00 00 00 00
  $  REQ045:7E0 03 22 01 97 00 00 00 00       $  ANS045:7E8 03 22 01 97 00 00 00 00
  $  REQ046:7E0 03 22 01 DD 00 00 00 00       $  ANS046:7E8 03 22 01 DD 00 00 00 00
  $  REQ047:7E0 03 22 01 F5 00 00 00 00       $  ANS047:7E8 03 22 01 F5 00 00 00 00
  $  REQ048:7E0 03 22 01 F6 00 00 00 00       $  ANS048:7E8 03 22 01 F6 00 00 00 00
  $  REQ049:7E0 03 22 01 F7 00 00 00 00       $  ANS049:7E8 03 22 01 F7 00 00 00 00
  ...
  $  024.气缸盖的机油温度                              $    $  degC        $    $  ANS024.BYTE004  $  y=(x1*0x100+x2)*0.0234375-273;
  ...
  $  041.油泵继电器状态                                $    $  /           $    $  ANS041.BYTE004  $  if(((x1>>0)&1))y=@0001;else y=@0000;
  $  042.空调离合器状态                                $    $  /           $    $  ANS042.BYTE004  $  if(((x1>>0)&1))y=@0001;else y=@0000;
  $  043.风扇实际输出占空比                            $    $  ％          $    $  ANS043.BYTE004  $  y=x1*0.390625;
  $  044.发动机运行时间                                $    $  s           $    $  ANS044.BYTE004  $  y=(x1*0x1000000+x2*0x10000+x3*0x100+x4)*1;
  $  045.MIL(故障指示灯)故障后的行驶里程               $    $  km          $    $  ANS045.BYTE004  $  y=(x1*0x100+x2)*1;
  $  046.主继电器状态                                  $    $  /           $    $  ANS046.BYTE004  $  switxh(x) 0x00:y=@0c62;0x01:y=@0c63;
  $  047.GPF(汽油机颗粒捕集器)碳载量模拟               $    $  G           $    $  ANS047.BYTE004  $  if(x1>0x80) y=((x1*0x100+x2)-65536)*0.001; else y=((x1*0x100+x2)*0.001);
  $  048.GPF（汽油机颗粒捕集器）碳载量测量             $    $  G           $    $  ANS047.BYTE006  $  if(x1>0x80) y=((x1*0x100+x2)-65536)*0.001; else y=((x1*0x100+x2)*0.001);
  $  049.灰分因子：基于CCF计算ASh(灰分)系数            $    $  /           $    $  ANS048.BYTE004  $  if(x1>0x80) y=((x1*0x100+x2)-65536)*0.001; else y=((x1*0x100+x2)*0.001);
  $  050.用于计算Ash(灰分)的燃油消耗量                 $    $  L           $    $  ANS048.BYTE006  $  if(x1<0x80)y=(x1*0x100+x2)*10;else y=(x1*0x100+x2-0x10000)*10;
  $  051.用于计算Ash(灰分)的发动机运行时间             $    $  s           $    $  ANS048.BYTE008  $  if(x1<0x80) y=(x1*0x1000000+x2*0x10000+x3*0x100+x4);else y=((x1*0x1000000+x2*0x10000+x3*0x100+x4)-0xFFFFFFFF);
  $  052.GPF （汽油机颗粒捕集器）传感器自学习值1       $    $  hPa         $    $  ANS049.BYTE004  $  if(x1<0x80)y=(x1*0x100+x2)*0.0390625; else y=(x1*0x100+x2-0x10000)*0.0390625;
  $  053.GPF（汽油机颗粒捕集器）传感器自学习值2        $    $  hPa         $    $  ANS049.BYTE006  $  if(x1<0x80)y=(x1*0x100+x2)*0.0390625; else y=(x1*0x100+x2-0x10000)*0.0390625;

...
读数据流: 
  ...
  $  REQ004:7D2 03 22 32 04 00 00 00 00       $  ANS004:7DA 03 22 32 04 00 00 00 00
  $  REQ005:7D2 03 22 32 05 00 00 00 00       $  ANS005:7DA 03 22 32 05 00 00 00 00
  $  REQ006:7D2 03 22 32 07 00 00 00 00       $  ANS006:7DA 03 22 32 07 00 00 00 00
  $  REQ007:7D2 03 22 32 08 00 00 00 00       $  ANS007:7DA 03 22 32 08 00 00 00 00
  $  REQ008:7D2 03 22 32 0A 00 00 00 00       $  ANS008:7DA 03 22 32 0A 00 00 00 00
  $  REQ009:7D2 03 22 32 0B 00 00 00 00       $  ANS009:7DA 03 22 32 0B 00 00 00 00
  $  REQ010:7D2 03 22 32 0C 00 00 00 00       $  ANS010:7DA 03 22 32 0C 00 00 00 00
  $  REQ011:7D2 03 22 32 0D 00 00 00 00       $  ANS011:7DA 03 22 32 0D 00 00 00 00
  ...
  $  003.扭矩输出比值                  $    $  %         $    $  ANS003.BYTE004  $  y = X1 *(100/255);
  $  004.转向速度                      $    $  deg/s     $    $  ANS004.BYTE004  $  y = (X1*256+x2) *0.1;
  $  005.方向盘扭矩                    $    $  Nm        $    $  ANS005.BYTE004  $  if(x1>=0x80)y = (X1*256+x2-0x10000)*0.01;else y=(X1*256+x2)*0.01;
  $  006.电机输出扭矩                  $    $  Nm        $    $  ANS006.BYTE004  $  if(x1>=0x80)y = (X1*256+x2-0x10000)*0.01;else y=(X1*256+x2)*0.01;
  ...
  $  008.电机位置                      $    $  deg       $    $  ANS008.BYTE004  $  y = (X1*256+x2)*(360/4096);
  $  009.电机速度                      $    $  rpm       $    $  ANS009.BYTE004  $  if(x1>=0x80)y=x1*256+x2-0x10000;else y=x1*256+x2;
  $  010.左端缓冲机制状态              $    $  /         $    $  ANS010.BYTE004  $  if((x1&0x03)==0x00)y=@0130;else if((x1&0x03)==0x01)y=@0387;else if((x1&0x03)==0x02)y=@0388;else if((x1&0x03)==0x03)y=@0209;  else y=@000b;
  $  011.右端缓冲机制状态              $    $  /         $    $  ANS010.BYTE004  $  if((x1&0x0c)==0x00)y=@0130;else if((x1&0x0c)==0x04)y=@0387;else if((x1&0x0c)==0x08)y=@0388;else if((x1&0x0c)==0x0c)y=@0209;  else y=@000b;
  $  012.系统状态                      $    $  /         $    $  ANS011.BYTE004  $  if(x1==0x00)y=@0389;else if(x1==0x01)y=@038a;else if(x1==0x02)y=@038b;else if(x1==0x03)y=@038c;else if(x1==0x04)y=@038d;else if(x1==0x05)y=@038e;else if(x1==0x06)y=@038f;else if(x1>=0x07&&x1<=0xFE)y=@0209;else y=@06e3;

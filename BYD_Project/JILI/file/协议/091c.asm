...
读数据流: 
  ...
  $  009.刹车灯开关                    $    $  /         $    $  ANS006.BYTE004  $  switxh(x)0x00:y=@0000;0x01:y=@0001;default:y=@0162;
  $  010.主缸压力                      $    $  bar       $    $  ANS007.BYTE004  $  if((x1*0x100+x2)<0x7FFF)y=(x1*0x100+x2)*0.0153;else if((x1*0x100+x2)>0x7FFF) y=((x1*0x100+x2)-0x10000)*0.0153;else y=@0132;
  $  011.转向角                        $    $  deg       $    $  ANS008.BYTE004  $  if((x1*0x100+x2)<0x7FFF)y=(x1*0x100+x2)*0.1;else if((x1*0x100+x2)>0x7FFF) y=((x1*0x100+x2)-0x10000)*0.1;else y=@0132;
  $  012.横摆角                        $    $  rad/s     $    $  ANS009.BYTE004  $  if((x1*0x100+x2)<0x7FFF)y=(x1*0x100+x2)*0.00213;else if((x1*0x100+x2)>0x7FFF) y=((x1*0x100+x2)-0x10000)*0.00213;else y=@0132;
  $  013.横向加速度                    $    $  m/s^2     $    $  ANS010.BYTE004  $  if((x1*0x100+x2)<0x7FFF)y=(x1*0x100+x2)*0.02712;else if((x1*0x100+x2)>0x7FFF) y=((x1*0x100+x2)-0x10000)*0.02712;else y=@0132;
  $  014.纵向加速度                    $    $  m/s^2     $    $  ANS011.BYTE004  $  if((x1*0x100+x2)<0x7FFF)y=(x1*0x100+x2)*0.02712;else if((x1*0x100+x2)>0x7FFF) y=((x1*0x100+x2)-0x10000)*0.02712;else y=@0132;
  $  015.电子驻车状态                  $    $  /         $    $  ANS012.BYTE004  $  switxh(x)0x00:y=@0018;0x01:y=@0019;0x02:y=@0f0c;default:y=@0209;
